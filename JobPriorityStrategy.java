public class JobPriorityStrategy<E> implements HeuristicStrategy<E>{
    ...
    public int findHeuristic(Job<E> j, State<E> s) {
        ...
        int f = h - edgeWeight;
        //if next not will complete a job, minus the entire edge cost and return smallest value for heuristic

        if (s.containsJob(j)) {
            return f;
        } else if (s.containsJobStart(j.getEnd())) {
            //if next job is the start of a job, minus half the edge cost and return
